...
批量实时地向 AE 服务器传输数据，不需要搭配传输工具。**在长时间网络中断情况下，有数据丢失的风险，不建议在正式环境中使用。优点在于使用简单无需搭配上报工具；缺点在于数据没有持久化存储，仅在内存中做缓存，如果网络不稳定缓存数据超过缓存区上限后会丢失数据。**

#### **BatchConsumer 性能指标如何？适合在什么场景下使用？**

BatchConsumer适合中小数据量，且日志上报服务器和AE集群内网打通时使用。
...
#### **在程序中调用了 `track()` 或者 `user_set()` 方法， 为什么在 AE 后台没有看到数据？**
