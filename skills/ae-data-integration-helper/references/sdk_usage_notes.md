...
2. **缓存上报策略**
  - Android、iOS 原生 SDK 以及底层调用 Android、iOS SDK 的游戏引擎 SDK 和跨平台 SDK，默认采用数据缓存和批量上报策略，上报时机默认 30条/30秒，即设备本地缓存达到 30 条数据或每隔 30 秒触发数据上报。可以在 AE Web "项目管理"页面调整上报频率，但如果上报过于频繁（比如 1条/1秒）会导致客户端系统 CPU、内存、网络等开销增加，引发设备过热、卡顿等问题。
...
4. **属性类型**
  - 对于 AE 预置属性（比如 `#account_id`、`#time` 、`#ip` 等），属性类型是 AE 系统固定设置的，预置属性清单详见预置属性与系统字段。
  - 对于自定义属性（除 AE 预置属性外的属性），属性类型是 AE 系统根据属性值自动识别的，第一条携带该属性的数据中的属性值决定了属性的类型；
  - 如果上报数据中属性值与 AE 系统的属性类型不一致，系统会尝试强制转换，如果转换成功该属性会入库，如果转换失败**该属性会被丢弃**；
...
5. **时间校准**
  ...
  - 客户端上报，AE 接收前10天至后3天时间范围的数据，超出时间范围的**数据会被丢弃**；
...
7. **敏感信息采集**
  - 由于法律要求，通常需要向用户展示隐私协议并获取许可后才能初始化 AE SDK 开启数据采集，请遵循各国法律和各应用商店要求披露和采集敏感信息；
...
10. **应用出海**
  - 如果应用投放海外，建议 AE 集群就近部署，避免跨墙等原因影响数据上报；
  - 如果应用全球投放，或数据需要从海外上报至部署在国内的 AE 集群，建议在应用主要投放地区就近部署一组或多组转接点，避免数据丢失和延迟；
  - 对于 SAAS 客户，AE SAAS 已经在全球多地部署转接点，请使用 AE SAAS 的海外上报地址 `https://global-receiver-ta.thinkingdata.cn`。
...
6. **IP、国家、地区信息**
  - AE 数据中的国家地区信息根据数据的ip信息解析，客户端上报 AE 会自动获取和存储IP信息，服务端上报需要在数据中添加"#ip"后 AE 才会处理；
...
1. **用户 ID 一致**
  ...
  - 如果使用单账号多设备分析体系，需要保证数据的`#account_id`相同且数据中带有`#distinct_id`，避免出现用户被割裂为多个 AE 用户的情况；
