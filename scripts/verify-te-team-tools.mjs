...
const commandsDir = path.join(ROOT, 'src/commands/te-team');
...
if (commandSet.size !== commands.length) {
  fail('duplicate team command names found in source files');
}

const EXPECTED_COUNT = 14;
if (commands.length !== EXPECTED_COUNT) {
  fail(`team tool count mismatch: expected ${EXPECTED_COUNT}, got ${commands.length}`);
}

const help = spawnSync('npx', ['tsx', 'src/index.ts', 'team', '--help'], {
  cwd: ROOT,
  encoding: 'utf-8',
});
...
console.log(`OK: verified ${commands.length} team tools are registered and aligned.`);
