...
`flow`、`task`、`setting`、`config` 四个旧 te-engage MCP 命令目录已全部迁移完成：45 个旧命令均有 Capability Gateway 等价入口，其中 33 个高频、稳定契约作为 L2 资源命令，12 个低频报表或复杂查询保留为 L3 Capability。te-cli 已删除所有旧 `engage +...` 命令及 MCP 路由注册，不保留 alias；Hermes 原 MCP Tool、Service 和注册全部保留。
...
| flow | 12 | 8 | 4 | 完成 |
| task | 10 | 6 | 4 | 完成 |
| setting | 13 | 12（含 4 个既有 scene 能力） | 1 | 完成 |
| config | 10 | 7 | 3 | 完成 |
| 合计 | 45 | 33 | 12 | 完成 |
...
- L3：`engage-setting.query.cancel`。
...
3. 对每个 L3 报表先执行 `capability get`、`dry-run`、`run`，记录 `request_id`，验证超时、重复请求、并发请求和取消行为。
...
4. 四类报表覆盖 flow UUID/ID、时间区间、节点维度、AB 节点、多时区、空结果、大结果及 request_id 取消。
...
4. 四类 task-data 报表覆盖 overview/detail/metric/experiment 维度、时间区间、指标集合、多语言、多时区、大结果和取消。
...
3. `engage-setting.query.cancel` 用真实长查询 request_id 验证运行中取消、重复取消、已完成/不存在请求和非 UUID 输入。
...
4. 三类 config 报表覆盖 config/template/strategy slice、template 与 strategy 过滤互斥、至少两个策略比较、日期边界、时区、空/大结果、request_id 延续及取消。

## 已知边界

- 自动化验证未连接真实 Hermes 数据源，因此权限矩阵、状态机副作用、跨项目模板复制和长查询取消必须按上述清单人工验证。
