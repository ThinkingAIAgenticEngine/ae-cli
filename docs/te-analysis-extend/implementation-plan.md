...
- 新增 35 个 extend 工具，CLI 入口仍在 `analysis` 域。
- 原 `analysis` 的 38 个工具保持不变（仍走 `analysis`）。
- 新增 35 个工具全部走 `analysis-extend`。
...
- 在 `docs/te-analysis-extend/expected-tools.txt` 维护基准名单（35 行，一行一个工具名，不带 `+`）。
- 校验必须同时满足：
  1. 基准名单与代码中标记 `mcpService: te_analysis_extend` 的命令集合双向全等（无缺失、无多余）。
  2. `ae-cli analysis --help` 能看到这 35 个命令，且名称逐项一致。
