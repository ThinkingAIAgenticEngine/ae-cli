...
- 新增 34 个 extend 工具，CLI 入口仍在 `analysis` 域。
- 原 `analysis` 的 38 个工具保持不变（仍走 `analysis`）。
- 新增 34 个工具全部走 `analysis-extend`。
...
- `createMcpCommand` 增加可选 `mcpService`（默认 `analysis`）：
  ...
  - 新 35 工具：传 `mcpService: te_analysis_extend`，走 `analysis-extend`
...
- 目录精简：不新增大批子目录，35 个命令并入现有分组目录：
  - `meta`（alert/metadata/virtual-meta/metric）—— 含 `delete_alert`、`delete_metric`
  - `dashboard`（dashboard-note/public-access）—— 含 `copy_dashboard`、`freeze_dashboards`、`list_spaces`、`move_dashboard`
  - 报表更新已迁移到 capability gateway：`analysis report update`
  - `model`（model-assist）
...
- 在 `docs/te-analysis-extend/expected-tools.txt` 维护基准名单（34 行，一行一个工具名，不带 `+`）。
- 校验必须同时满足：
  1. 基准名单与代码中标记 `mcpService: te_analysis_extend` 的命令集合双向全等（无缺失、无多余）。
  2. `ae-cli analysis --help` 能看到这 34 个命令，且名称逐项一致。
