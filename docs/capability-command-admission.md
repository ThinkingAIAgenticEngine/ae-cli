...
L1 面向完整业务任务。新增命令统一使用 `ae-cli <domain> <resource> <action>` 形态，不因内部编排、上传或轮询增加 `+` 前缀。L1/L2 是收录分类，不是用户必须辨认的命名差异；已有 `+` 命令保持兼容，不在新增功能时顺带迁移。L1 的行为不能由一次 capability 调用等价表达，并且至少提供以下一种编排价值：
