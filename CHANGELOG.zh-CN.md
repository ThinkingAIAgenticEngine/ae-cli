...
* 新增 `auth set-token`，支持隐藏输入或通过标准输入导入从外部访问管理复制的 CLI Token；写入前校验目标 Host，并支持在保留同一 Host 其他账号的情况下添加凭证。
* 加固跨运行环境的凭证兼容和安全存储：兼容新旧凭证文件、保护无法解密的凭证，并在双文件写入部分失败时自动回滚，避免错误导入或误判掉线。
* 新增 `config show`，返回当前 Host 的精简生效配置和知识库路由模式；配置按 Host 与账号缓存，在配置不可用时停止自动知识库路由。

#### Agent 与资产治理

* 新增应用内通知的收件人查询、发送、列表、详情、未读计数和已读标记命令，并补充幂等重试、敏感内容脱敏和权限边界。
* 新增治理推荐资产的自动审核与认证流程，支持扩容处理、失败资产转人工审核、保留父资产上下文及完整审核痕迹，并由 CLI Agent 处理语义重复。
* 为 Capability 请求透传和记录 CLI Agent 上下文，优化知识库发现、资产搜索召回及当前项目记忆路由。

#### 分析能力

* 完善事件、漏斗、路径、留存、收入、排名、间隔、热力图、标签等分析模型的参数、筛选、时间、分组和结果解释契约，并补齐复合筛选的递归校验与无损回写规则。
* 修复报表筛选写入、公式字段回读、描述清空、指标与分析实体区分、看板导出缓存参数以及元数据导出认证范围等问题。
* 完善资产治理异步导出的等待与下载流程，并修复分析校验、资产导出和临时文件清理问题。

#### DataOps 与知识库

* 新增工作流自定义参数管理，并将任务参数查询合并到工作流概览；统一表和视图生命周期、实例监控及状态筛选指引。
* 新增数据表单字段的添加、修改和删除能力，以及表和视图回收、回收站查询和彻底删除流程，提供语义预览、明确确认和结构化执行结果。
* 新增知识库编译规则导入全流程，包括下载、上传、校验、优化、差异查看、应用和放弃，并提供版本、哈希、异步任务及冲突保护。
* 按公司配置引导项目知识库发现，收敛项目语义入口，并修复个人语义偏好消费所需的标题与路由信息。

#### Skill 协作

* 为十个系统 Skill 增加去中心化协作规范、共享契约和独立分发校验，并纳入发布门禁，避免协作规则在同步过程中遗漏或漂移。

#### 6.1 独有

* 新增会话分析模型，支持按间隔、开始事件、开始/结束事件和会话 ID 切分会话，并支持会话或步骤视图、指标、分组、分桶及会话筛选。
* 新增 `analysis drilldown-session-details run`，可从会话分析结果下钻查看具体会话或步骤明细。
* 加固项目语义知识库增量刷新：持久化语义计划、片段依赖和远端刷新基线，只更新受影响的来源文件，保护人工修改，并在无内容变化时跳过编译。
...
* 泛化分析、数据集成和运营 Skill 中的项目名称、事件、属性及业务示例，使用通用示例或占位符，并明确审核说明应引用当前资产定义中的实际事件和属性。
* 移除埋点代码生成 Skill 中固定的飞书埋点方案链接，改为使用用户提供的埋点方案文档作为输入。
...
* 完善分析模型 Skill 的输入约束，要求按所选模型构造参数，仅在模型支持时传入时间范围和时间粒度。
* 明确用户属性分析的参数层级及历史日期查询边界，避免将当前属性统计误用于回答历史日期问题。
* 补充路径分析的必填事件范围规则，明确事件列表必须包含起始事件，并说明节点和连线按路径层级组织的二维数组结构及遍历方式。
...
#### 分析与治理
* 重构 `ae-analysis` 的执行与结果恢复流程，默认直接使用有效分析结果并保留退出状态，精简命令索引与模型参考文档；同时加强元数据、已保存定义和结果文件的复用，并补充自然日／周／月窗口及公式报表回写约束。
* 新增 Agent 审核与治理推荐 CLI 闭环，覆盖批次列表、详情、证据、评审、重试、推荐导出和提交页面，并支持推荐分批处理、保留看板空间信息及项目语义资产包导出。
* 支持设置看板个人默认筛选，并修复 SQL 报表 Selector 运行参数、统一资产搜索、虚拟事件快照日期、项目成员创建和角色更新等契约问题。

#### 埋点迁移与数据集成
* 新增 `ae-migrate-tracking-code` Skill，支持从 Amplitude、Firebase／GA4、Mixpanel、神策等第三方 SDK 迁移至 AE，覆盖 Web、服务端、Android、iOS、React Native、`dataLayer` 和封装调用等代码形态，并完善双写隔离、公共属性、身份映射、合规开关与旧代码删除确认。
* 统一用户属性代码生成规则，支持 `user_append`，并确保 `user_append`／`user_unset` 等操作按正确方式生成和迁移。
* 新增飞书多维表格、飞书电子表格和手动导出文件的数据接入流程，补充快照、复用、UE 映射及上传规则；多维表格使用保留键 `__record_id` 保存稳定记录 ID，并在业务字段冲突时中止，避免去重键被覆盖。

#### 知识库
* 支持替换和恢复知识库文件及 ZIP 数据源，保留目录结构和版本校验；新增飞书来源更新检查能力，便于识别远端文档变化。

#### 页面上下文与兼容性
* 新增 `context +current`、`ae-current-context` 及页面上下文业务 Skill，支持 Agent 按需读取当前产品页面上下文，并通过发布门禁确保内部页面词典不会进入公网包。
* 修复 CLI 的 ESM 兼容性与依赖告警，提升数据集成和 XLSX 处理链路的运行稳定性。

#### 6.1 独有
* 新增 `experiment external-experiment save-submit`，支持一体化创建或更新并提交外部实验，校验实验分组、唯一对照组、主指标、分析分桶和实验时间；更新已提交实验时要求保留稳定分组 ID。
...
#### Agent 与认证
* 补齐 CLI-token 自定义 Agent 依赖预检、同公司接收人查询、不可变快照分享、收发列表、接受／拒绝及审批快照预览（含驳回后预览）。
* 提交公司复用 `agent.publish@1` 通用审批流程，补充逐接收人结果、版本与幂等校验、错误恢复及仅本地预览的 dry-run 边界文档。
* 支持同一 Host 安全保存和管理多个账号，新增账号追加、列表、切换与定向退出能力，并支持通过 `ae-cli auth` 或 `auth use` 交互式切换当前账号；凭据继续只持久化 CLI Token，并兼容旧版凭据格式。
* 新增 `agent +get-agent-context` 和本地 Agent 使用 Skill，支持获取 Agent 配置、依赖及当前用户可见的 MCP 市场连接配置，并修复本地 Agent 独立使用时的接入指引和依赖请求地址。
* 为 Agent 自动化的创建、列表和更新补充工作空间参数，支持显式选择工作空间或继承当前会话工作空间，避免自动化被错误创建或查询到个人默认空间。

#### 知识库
* 对齐知识库 External API 的作用域、目标解析和错误返回契约，并支持向公司知识库导入编译后的只读快照及查询导入状态。
* 支持将 ZIP 作为保留目录层级的可编辑父来源上传，新增目录列表、子文件读取、更新和递归删除能力，并通过 revision 校验防止覆盖并发修改。
* 新增不可变发布版本历史查询、来源与目录快照查看、文件下载、版本差异比较，以及带幂等和状态查询的高风险异步回滚；回滚会创建新的当前版本并保留历史快照。
* 统一 `kb +grep` 的页级 `results` 返回与检索纪律，修正帮助文档和 catalog 页面使用规则；同时移除 `kb +ask` 的轮次上限参数、保留类型化 Ask 错误，并明确来源删除后的发布状态语义。

#### 数据集成
* 放开数据集成和埋点方案中的大写事件名限制，保持方案生成、映射、转换与 Excel 输出规则一致。
* 新增维度数据向 `ae-metadata` 路由的判断规则，并补充从 XLSX、PPTX 等 PRD 文档读取埋点需求的流程。

#### 分析
* 支持在创建和更新用户标签时配置自动刷新，并允许看板便签仅更新显式传入的字段，避免覆盖未修改内容。
* 新增跨源资产配置的 L3 能力发现与调用流程，覆盖模板下载、工作簿上传、业务预览、导入、运行状态查询及部分写入证据处理。
* 加强漏斗步骤筛选参数校验，明确事件属性字段和值类型；补充指标值标签分位数契约及分析失败证据规则，避免把进程正常退出、策略拒绝或查询失败误判为业务成功。

#### 兼容性
* 回退 `archiver` 至兼容版本，恢复数据集成交付包和沙盒工具包的 ZIP 打包能力，并保持 Node.js 20 运行兼容。
