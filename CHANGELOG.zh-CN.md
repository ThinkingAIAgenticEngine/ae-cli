...
#### 数据集成
* 增强本地文件检查与转换，覆盖 Excel 日期与 1904 日期系统、无缓存公式、合并单元格、隐藏行列、命名空间工作表、标题行和汇总行，并新增重复业务键、值频及数值分布检测
* 收紧文件级数据质量分级、UE 路由、数据粒度、多来源合并及隔离行修复规则，防止累计快照、聚合报表或范围重叠的数据被误当作普通事件上传

#### 知识库
* 新增 `kb +import` 与 `kb +import-status`，支持异步导入编译后的 Markdown ZIP，并生成可查询、检索和问答的个人只读知识库快照；Schema 生成支持一次性自定义指令及显式强制重启
* 新增 `kb +list-sources` 来源列表，并支持通过稳定来源 ID 执行 `kb +rm-source --id`，避免根据文件名或 URL 猜测删除目标

#### DataOps 与运营
* 新增 DataOps 补数作业完整生命周期管理，支持发现可补数流程、创建和更新草稿、运行、查询详情、搜索、停止、重新运行及删除作业
* 在运营任务保存前校验渠道与受众类型的兼容性：服务端渠道仅支持自定义或已有分群，`client_push` 仅支持自定义或全量用户，提前拦截无效组合

#### 系统与渠道
* 将 `ae-cli system` 从 59 条扩展到 71 条命令，覆盖九类渠道配置、Endpoint 路由、WhatsApp Web 扫码生命周期、飞书用户绑定，以及支持逐人 Agent 覆盖和逐项结果的 1-100 人韧性批量分配
* 新增 `ae-system` 渠道管理工作流，分离渠道与人员确认阶段，通过 `lark-cli` 解析飞书身份，默认创建群聊就绪绑定，并在不自动回滚的前提下保留逐项重试证据

#### 分析与平台
* 补充用户标签的动态时间范围语义，支持今天、本周、本月、本季度、本年、上一周期及固定日期至今天或昨天等范围
* 将用户分群更新升级为高风险写操作，并禁止通过更新修改分群类型或分析主体；同时修复项目时区更新的请求字段校验，明确元数据创建能力共用的编辑权限
* 兼容尚未提供 CLI Token 状态校验接口的旧版服务，同时继续拒绝明确失效或过期的 Token
* 新增面向 AI Agent 的 CLI 安装与升级指南，将 Node.js 最低版本统一为 20，清理安装时的弃用依赖告警、过期构建产物及旧版 MCP 接入 Skill

#### 6.1 独有：实验
* 加固实验指标与流程 A/B 指标的语义校验：指标属性比较统一使用 `metricDefinition.filters`，校验聚合类型、必需属性、比较操作和值；流程 A/B 指标使用 `eventDefinition`，并拒绝与旧版 `event` 字段混用
