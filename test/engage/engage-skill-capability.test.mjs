...
assert.match(saveTaskRef, /`touch_cycle_num` and[\s\S]*`touch_cycle_num_unit`/);
assert.match(saveTaskRef, /Verified A\/B Task Template/);
assert.match(saveTaskRef, /group tuple[\s\S]*must be identical/);
assert.match(buildTaskSaveGuideRef, /filterPropertySelectTypes/);
assert.match(buildTaskSaveGuideRef, /`datetime` is excluded/);
assert.match(buildTaskSaveGuideRef, /`requiredMainGoalFields` and[\s\S]*`touchCycleRule`/);
assert.match(buildTaskSaveGuideRef, /`exp_config\.exp_group_list` and `group_content_list`/);
